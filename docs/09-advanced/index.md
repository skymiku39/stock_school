...
??? question "1.（概念題）老手專區與入門章最大的差別？"
    參考答案：差在**系統與紀律**（研究流程、組合、多週期），不是多背名詞。

??? question "2.（判斷題）還沒固定一種主模式，就該先讀事件手冊當沖？"
    參考答案：不宜。先 [選定模式](../08-investing/choose-style.md) 並精熟，再進老手專區。

??? question "3.（情境題）想建立 thesis 與查證流程，優先讀哪篇？"
    參考答案：[研究流程](research-workflow.md)。
