...
| 誤區 | 正確做法 |
|------|----------|
| 有 thesis 就不用停損 | thesis 失效條件與停損要**同時寫** |
| 篩選階段就看分 K | 先用日線／週線與基本面粗篩，見 Step 1 |
| 評分高分 = 可買 | 分數是粗篩，須拆因子查證，見 [評分量表](../03-tables/scoring.md) |
| 複盤只記盈虧 | 要記**是否遵守計畫**與偏離理由 |

## 自我檢查

??? question "1.（概念題）thesis 的「失效條件」要寫什麼？"
    參考答案：可觀測、可執行的條件（如營收連續 MoM 負、法人連賣 N 日、跌破關鍵價），觸發時必須重新評估或出場。

??? question "2.（判斷題）執行階段可以「加碼攤平」嗎？"
    參考答案：除非進場前計畫已寫好加碼條件，否則虧損後無計畫加碼屬常見錯誤。

??? question "3.（情境題）候選池有 12 檔，你每週該先做什麼？"
    參考答案：依模式選框架評分粗篩 → 對最低因子查證 → 精煉到 5～15 檔並更新 thesis，而非全部下單。
