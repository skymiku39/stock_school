...
??? question "1.（概念題）風控四層的順序是什麼？"
    參考答案：**資金 → 成本 → 停損 → 紀律**；見本篇重點回顧。

??? question "2.（判斷題）停損設好就可以不用管資金配置？"
    參考答案：不行。先定能虧多少（[資金配置](capital.md)），再談停損與紀律。

??? question "3.（情境題）除息當天股價跳空下跌，先看哪份手冊？"
    參考答案：[突發狀況手冊](emergency-playbook.md#除息誤買)，勿誤當利空殺出。
