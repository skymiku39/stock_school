...
??? question "1.（概念題）「心態錯配」是什麼？"
    參考答案：用 A 模式的**方法**進場，卻用 B 模式的**持有／出場心態**（如當沖卻等它回來、定額卻每天追漲殺跌）。

??? question "2.（判斷題）選模式時應以「哪種聽起來最賺」為準？"
    參考答案：應以 [你能執行的紀律](choose-style.md#五個自問) 為準——時間、隔夜風險、盯盤頻率。

??? question "3.（情境題）你選了 ETF 定額，大盤連跌三週，你會每天看分 K 加碼嗎？"
    參考答案：定額模式應依預先寫好的加碼規則與閒錢紀律，而非情緒驅動；見 [0050 專章](etf-passive-dca.md) 與 [案例](../07-cases/etf-dca-drawdown.md)。
