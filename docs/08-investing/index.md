...
??? question "1.（概念題）「模式」與「分析三支柱」各回答什麼？"
    參考答案：模式決定**看什麼、看多久**；三支柱決定**看什麼面向**（基本面／技術／籌碼）。

??? question "2.（判斷題）評分表當沖分高、長線分低，代表一定要當沖？"
    參考答案：否。只代表短線技術面較強；仍須對齊你的**時間與心理**，見 [如何選模式](choose-style.md)。

??? question "3.（情境題）選定中線後，看評分表該盯哪一欄？"
    參考答案：**中線分數**；勿混用當沖停損規則，見本篇「與評分表、時間框架的關係」。
