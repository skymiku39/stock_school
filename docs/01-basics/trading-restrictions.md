...
??? question "1.（概念題）注意股、處置股、全額交割差在哪？"
    參考答案：注意股 = 異常警訊；處置股 = 交易規則改變（如分時撮合）；全額交割 = **不得信用**買進。

??? question "2.（判斷題）處置股期間可以像一般股一樣用融資追漲？"
    參考答案：通常不行或極難成交；處置期常禁融資，且分時撮合下流動性變差。

??? question "3.（情境題）當沖前發現標的進入處置，你該怎麼做？"
    參考答案：宜避開或大幅減碼；見 [處置股案例](../07-cases/disposal-stock-trap.md) 與 [當沖風控案例](../07-cases/day-trade-risk.md)。
