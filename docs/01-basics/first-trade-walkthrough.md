...
??? question "1.（概念題）台股買進後，款項大約何時從交割戶扣款？"
    參考答案：**T+2 交割日**（非下單當下，但須事先備足款）。

??? question "2.（判斷題）第一筆交易可以省略停損，因為金額小？"
    參考答案：仍應養成習慣。事前設 [停損](../06-risk/stop-loss.md)，並確認無 [處置標記](trading-restrictions.md)。

??? question "3.（情境題）完整流程七步是什麼？"
    參考答案：開戶 → 綁銀行 → 入金 → 下單 → 成交 → **T+2 交割** → 對帳。
