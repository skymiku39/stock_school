...
| 情境 | 年化內扣費 | 淨報酬 | 10 年後約略（示意） |
|------|------------|--------|---------------------|
| A | 0.4%（大盤 ETF 量級） | 7.6% | 約 **208 萬** |
| B | 1.5%（部分主動商品量級） | 6.5% | 約 **188 萬** |
| **差距** | 差 1.1%／年 | — | 約 **20 萬** |
...
??? question "1.（概念題）ETF 三層費用中，哪一層最隱形、長期最關鍵？"
    參考答案：**內扣管理費**（每日自淨值扣除）。

??? question "2.（判斷題）0050 與 006208 追蹤同一指數，應兩檔都定額？"
    參考答案：不必。同指數**選一檔定額**即可，比較內扣費與規模。

??? question "3.（手算題）8% 毛報酬、10 年，0.4% vs 1.5% 內扣終值差約多少（教學表）？"
    參考答案：約 **208 萬 vs 188 萬**，差約 **20 萬**（示意複利）。
