...
| 代號 | 通常 4 碼 | 常見 4～6 碼（如 0050、00878、006208） |
...
0050 是追蹤**台灣 50** 的被動型大盤 ETF（4 碼、盤中像股票買賣），常見用法是閒錢 + 定期定額。
...
| 誤區 | 正確認知 |
|------|----------|
| ETF = 一定比個股安全 | 仍隨指數或策略波動，非保本 |
| 主動 ETF 加碼 = 買點 | 持股變化是**研究線索**，非買賣指令 |
| 0050 = 定存替代品 | 有大盤回撤風險，見 [0050 專章](../08-investing/etf-passive-dca.md) |
| 只看代號不看費用 | 同指數 ETF 費率差異會影響長期報酬 |

## 自我檢查

??? question "1.（概念題）被動型與主動型 ETF 差在哪？"
    參考答案：被動型追蹤指數、持股隨指數調整；主動型由經理人選股，持股會變動。

??? question "2.（判斷題）買 ETF 就不用看財報？"
    參考答案：仍須了解追蹤什麼指數或策略、費用與折溢價；主動型更要看持股邏輯。

??? question "3.（情境題）新手想跟大盤、每月固定扣款，該讀哪一章？"
    參考答案：[被動 ETF 與定期定額](../08-investing/etf-passive-dca.md) 與 [案例](../07-cases/etf-dca-drawdown.md)。
