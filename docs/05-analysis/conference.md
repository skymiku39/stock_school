...
| 誤區 | 正確做法 |
|------|----------|
| 只看新聞標題 | 到 [MOPS 重大訊息](https://mops.twse.com.tw/mops/web/t05sr01_1) 讀**原文** |
| 法說樂觀就追 | 對照 [法人表](../03-tables/institutional.md) 言行是否一致 |
| 有公告 = 還沒反映 | 股價常領先；要判斷利多是否已反映在價格上 |
| 忽略 Q&A | 分析師追問的尖銳問題常藏風險 |

## 自我檢查

??? question "1.（概念題）法說會閱讀清單的第一項是什麼？"
    參考答案：看管理層有無**量化指引**（營收、毛利率、CapEx 等具體數字）。

??? question "2.（判斷題）口頭看多 + 法人連續賣超，該更放心還是更警覺？"
    參考答案：更警覺——話術與行為不一致，需深入查證是否結構性出貨。

??? question "3.（情境題）股價跳空，你第一步該做什麼？"
    參考答案：到 MOPS 查當日重大訊息 → 判斷利多／利空與是否已反映；見 [事件驅動查證](#事件驅動查證)。
