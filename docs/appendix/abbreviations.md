# 縮寫與英文對照

## 本篇你會學到

- 台股投資常見中英文縮寫
- 每個縮寫的中文說法與延伸連結

[← 基本資料書](reference-book.md) · [完整詞條](../02-glossary/dictionary.md)

!!! note "與詞典的分工"
    本表是**縮寫**的權威來源；[完整詞條總表](../02-glossary/dictionary.md) 已新增「英文」欄，兩者英文若有出入，一律以本表為準。

---

## 行情與交易

| 縮寫 | 英文 / 全名 | 中文 | 詳見 |
|------|-------------|------|------|
| IOC | Immediate or Cancel | 立即成交否則取消 | [交易流程](../01-basics/trading-flow.md) |
| T+2 | Trade date + 2 | 成交後第 2 日交割 | [交割](../01-basics/settlement-fees.md) |
| Tick | — | 逐筆成交明細 | [報價畫面](../01-basics/quote-screen.md) |
| OHLC | Open High Low Close | 開高低收 | [quotes](../02-glossary/quotes.md#開高低收) |

---

## 基本面

| 縮寫 | 英文 / 全名 | 中文 | 詳見 |
|------|-------------|------|------|
| EPS | Earnings Per Share | 每股盈餘 | [fundamentals](../02-glossary/fundamentals.md) |
| PER / P/E | Price Earnings Ratio | 本益比 | [PER](../02-glossary/fundamentals.md#per本益比) |
| PBR / P/B | Price Book Ratio | 股價淨值比 | [fundamentals](../02-glossary/fundamentals.md) |
| YoY | Year over Year | 年增（同比） | [fundamentals](../02-glossary/fundamentals.md) |
| MoM | Month over Month | 月增（環比） | [fundamentals](../02-glossary/fundamentals.md) |
| QoQ | Quarter over Quarter | 季增 | [fundamentals](../02-glossary/fundamentals.md) |
| ROE | Return on Equity | 股東權益報酬率 | [財報](../03-tables/financials.md) |
| IPO | Initial Public Offering | 初次上市 | [what-is-stock](../01-basics/what-is-stock.md) |
| MOPS | Market Observation Post System | 公開資訊觀測站 | [資料來源](data-sources.md) |

---

## 技術分析

| 縮寫 | 英文 / 全名 | 中文 | 詳見 |
|------|-------------|------|------|
| MA | Moving Average | 移動平均 / 均線 | [MA](../02-glossary/technical.md#均線-ma) |
| EMA | Exponential MA | 指數移動平均 | [ma](../04-charts/ma.md) |
| MACD | Moving Average Convergence Divergence | 指數平滑異同移動平均 | [MACD](../04-charts/macd.md) |
| RSI | Relative Strength Index | 相對強弱指標 | [RSI](../04-charts/rsi.md) |
| KD | Stochastic Oscillator | 隨機指標 | [KD](../04-charts/kd.md) |
| BB / 布林 | Bollinger Bands | 布林通道 | [bollinger](../04-charts/bollinger.md) |
| TA | Technical Analysis | 技術分析 | [三大支柱](../05-analysis/three-pillars.md) |

---

## 籌碼與市場

| 縮寫 | 英文 / 全名 | 中文 | 詳見 |
|------|-------------|------|------|
| ETF | Exchange Traded Fund | 交易所交易基金 | [ETF](../01-basics/etf-intro.md) |
| NAV | Net Asset Value | 基金淨值 | [共同基金入門](../01-basics/mutual-fund-intro.md) |
| DCA | Dollar-Cost Averaging | 定期定額 | [0050 與定期定額](../08-investing/etf-passive-dca.md) |
| ADR | American Depositary Receipt | 美國存託憑證 | [跨市場](../05-analysis/cross-market.md) |
| OI | Open Interest | 未平倉量（期貨） | [期貨輔助](../09-advanced/futures-signal.md) |
| TDCC | Taiwan Depository & Clearing | 集保（大戶持股） | [chips](../02-glossary/chips.md) |
| QE | Quantitative Easing | 量化寬鬆 | [基本面框架](../05-analysis/fundamental-framework.md) |
| CPI | Consumer Price Index | 消費者物價指數 | [macro](../09-advanced/macro-rotation.md) |
| GDP | Gross Domestic Product | 國內生產毛額 | [macro](../09-advanced/macro-rotation.md) |

---

## 損益與風控

| 縮寫 | 英文 / 全名 | 中文 | 詳見 |
|------|-------------|------|------|
| PnL | Profit and Loss | 損益 | [pnl](../02-glossary/pnl.md) |
| R/R | Risk Reward | 風險報酬比 | [risk](../02-glossary/risk.md) |
| DD | Drawdown | 回撤 | [risk](../02-glossary/risk.md) |

---

## 程式對照（選用）

學員詞 ↔ Stock Bot 欄位見 [dev-glossary](dev-glossary.md)（如 `symbol`、`pct_chg`）。

---

## 重點回顧

- 縮寫是溝通捷徑；不懂就查本表再點**詳見**。
- 公式 → [formulas](formulas.md)。
