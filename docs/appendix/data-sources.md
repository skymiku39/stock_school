...
| [GS10](https://fred.stlouisfed.org/series/GS10) | 10 年期公債殖利率（月） | 同上，月頻率 |
| [T10Y2Y](https://fred.stlouisfed.org/series/T10Y2Y) | 10 年減 2 年利差 | 殖利率曲線，負值（倒掛）常預示衰退 |
| [T10Y3M](https://fred.stlouisfed.org/series/T10Y3M) | 10 年減 3 月利差 | 紐約聯準銀行衰退機率模型基準 |
| [DFII10](https://fred.stlouisfed.org/series/DFII10) | 10 年期抗通膨債殖利率 | 實質利率代理；名目−實質≈預期通膨 |
...
| `/exchangeReport/TWT84U` | 每日漲跌停價、開盤競價基準（風控邊界） |
...
??? question "1.（概念題）查月營收、三大法人、集保大戶各應去哪？"
    參考答案：**MOPS**、**證交所／櫃買**、**TDCC 股權分散表**（見本篇表格）。

??? question "2.（判斷題）三大法人昨日大買，今天盤中一定會漲？"
    參考答案：否。法人資料 **T+1**，且單日不代表趨勢；見「使用原則」lag 說明。

??? question "3.（情境題）比較兩家同業 PER，口徑不同怎麼辦？"
    參考答案：改用**同一來源**、注意 N/A 防呆；見「使用原則」第 3 點與 [估值表](../03-tables/valuation.md)。
