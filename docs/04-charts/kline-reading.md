...
「大紅K」「十字線」「鎚子」是交易員之間共通的描述方式，便於溝通與查閱 [16 種型態](candle-patterns.md)。
...
| 誤區 | 正確認知 |
|------|----------|
| K線能預測突發事件 | 政策、災害等須看 [重大訊息](../05-analysis/conference.md) |
| 單一K線就夠 | 需搭配 [均線](ma.md)、[量價](volume-price.md)、籌碼 |
| 型態必準 | 盤整市假訊號多，宜等確認 |

## 自我檢查

??? question "1.（概念題）讀K線「三招」順序是？"
    參考答案：**顏色 → 實體 → 影線**（先結果、再過程）。

??? question "2.（判斷題）K線可以取代財報分析嗎？"
    參考答案：不行。K線只看價量，存股與價值投資仍需 [基本面](../05-analysis/three-pillars.md)。

??? question "3.（情境題）看到疑似反轉K線，實務上多一步什麼？"
    參考答案：對照趨勢位置與量，必要時**等次日確認**再寫停損計畫。
