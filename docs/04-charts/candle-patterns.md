...
| 誤區 | 正確認知 |
|------|----------|
| 看到鎚子就買 | 須在**低檔**且搭配量與次日確認 |
| 16 型態 = 買賣指令 | 是描述語言，非保證 |
| 忽略趨勢位置 | 同型態在高檔與低檔意義相反 |
| 一字線當一般訊號 | 多為漲跌停鎖死，情境特殊 |

## 自我檢查

??? question "1.（概念題）反轉型態（如鎚子）最依賴什麼？"
    參考答案：**高低檔位置** + 量 + 確認；見 [鎚子線案例](../07-cases/hammer-ma.md)。

??? question "2.（判斷題）高檔出現倒鎚，可以當成續漲訊號？"
    參考答案：不宜。倒鎚在高檔常代表上方賣壓，宜提高警覺。

??? question "3.（情境題）辨識型態後，實務上還要多哪兩步？"
    參考答案：對照 [成交量](../02-glossary/quotes.md#成交量) 與**次日／組合確認**，並寫停損假設。
