...
學完各指標後，如何**系統性整合**多種信號做出判斷？

→ **[圖表綜合評估框架](chart-evaluation.md)**：五層評估架構 + 信號匯流計分 + 多空記錄模板。

適合已掌握基礎指標、想建立**可重複的評估流程**的讀者。
...
=== "已有 K 線基礎"
    ...
    3. [手冊：接盤還是追高](catch-or-chase.md)（綜合判讀）
...
??? question "1.（概念題）K 線與「所有圖表」的關係？"
    參考答案：K 線是**價格圖的一種**；還有分時、量價、指標、籌碼等圖。

??? question "2.（判斷題）還沒選投資模式，可以先背 16 種 K 線型態？"
    參考答案：不宜。先選 [投資模式](../08-investing/index.md)，再決定圖表組合。

??? question "3.（情境題）當沖族在本章優先學哪幾種圖？"
    參考答案：**分時圖、1/5 分 K、量價**；見 [分時圖](intraday-charts.md) 與模式專章。
